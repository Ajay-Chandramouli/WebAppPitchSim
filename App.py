...
#external style sheet for the app
external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css'] #A nice stylesheet!
# engine object to connect to the SQL Database
engine = create_engine('mysql+mysqlconnector://{user}:{pwd}@{host}/{database}'
                       .format(user='root', pwd='usausb12', host='localhost', database='PitchMisalignment'))
...
#Descriptions realted to the data
...
#Now, the text to be used in the app is ready. Each text block is referred by a header as written in the .txt file.

#user options in the drop-down, radio buttons etc.
...
#Adding an image in Dash is not easy. It needs to be base64 encoded and then added. Solution found online :P
image_filename = path + '//'+ 'DTU10MWresized.png'
encoded_image = base64.b64encode(open(image_filename, 'rb').read())

#html layout of the app. Dash html components are Verrry similar to standard html components
app.layout = html.Div([
...
#App callbacks for backend calculations
...
#Data is loaded dynamically from SQL.
def dynamic_load(PitchLevel1,PitchLevel2,Turbxdd,Turbydd,Wsp,SeedInfo):
    Seed = (1,1) if SeedInfo ==1 else (1,0) # same seed if SeedInfo = 1, otherwise different seeds.
    Dict = {} #dataframes will be stored in this dict and each df will be labelled as per the simulation it represents.
    MainDict={} #Here the data will be json serialized so that it can be shared between callbacks
    #List of columns to be downloaded form SQL
    ...
    for n,PitchLevel in enumerate([PitchLevel1,PitchLevel2]):
        ...
        SQLQuery = 'SELECT * FROM {} WHERE Wsp = %s AND Seed = %s '.format(dbtable) #SQL query to be executed
        Dict[CaseOrder[n]]  = pd.read_sql(SQLQuery,con = engine,params=(Wsp,Seed[n]),columns=Colslist) # executing the query in MySQL

    for key in Dict.keys():
         MainDict[key] = Dict[key].to_json(orient='split') #serializing to json
